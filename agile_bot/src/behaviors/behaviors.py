...
class Behaviors:
    ...
    def close_current(self):
        if self._current_index is not None and self.current:
            # Close the current action first (marks it complete)
            if self.current.actions and self.current.actions.current:
                self.current.actions.close_current()
            # Then advance to next behavior if available
            next_behavior = self.next()
            if next_behavior:
                self._current_index += 1
                self.save_state()
    ...
    def save_state(self):
        ...
        # Also save current_action if actions are initialized and have a current action
        if self.current.actions and self.current.actions.current:
