...
class BotPanel {
  ...
  constructor(panel, workspaceRoot, extensionUri) {
    try {
      ...
      // Initialize BotView with null CLI (will spawn its own)
      this._botView = null;
  ...
  static createOrShow(workspaceRoot, extensionUri) {
    try {
      console.log(`[BotPanel] createOrShow called - workspaceRoot: ${workspaceRoot}, extensionUri: ${extensionUri}`);
      ...
      console.log("[BotPanel] BotPanel created successfully");
    } catch (error) {
      console.error(`[BotPanel] ERROR in createOrShow: ${error.message}`);
      console.error(`[BotPanel] ERROR stack: ${error.stack}`);
  ...
  _readPanelVersion() {
    try {
      ...
      console.error("Failed to read panel version:", error);
      return null;
    }
  }
  ...
  async _update() {
    try {
      ...
      // Initialize BotView if needed
      if (!this._botView) {
        console.log("[BotPanel] Creating initial BotView");
        try {
          // Get initial bot status
          console.log("[BotPanel] Creating temporary BotView for status check");
          const initialBotView = new BotView({}, null, this._workspaceRoot, null, this._panelVersion, webview, this._extensionUri);
          console.log("[BotPanel] Executing 'status' command");
          const initialStatus = await initialBotView.execute('status');
          console.log("[BotPanel] Status received, creating main BotView");
        ...
        console.log("[BotPanel] Refreshing existing BotView");
        // Refresh bot status
        await this._botView.refresh();
      }
      
      console.log("[BotPanel] Rendering HTML");
      // Render HTML using BotView
      const html = this._getWebviewContent(this._botView.render());
