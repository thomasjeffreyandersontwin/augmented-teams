...
```
story_io/
Γö£ΓöÇΓöÇ __init__.py                    # Package exports (StoryIODiagram, Epic, Feature, etc.)
Γöé
Γö£ΓöÇΓöÇ story_io_component.py          # Base class for all components
Γö£ΓöÇΓöÇ story_io_diagram.py            # Main diagram orchestrator
Γö£ΓöÇΓöÇ story_io_epic.py               # Epic domain component
Γö£ΓöÇΓöÇ story_io_feature.py            # Feature domain component
Γö£ΓöÇΓöÇ story_io_story.py              # Story domain component
Γö£ΓöÇΓöÇ story_io_user.py               # User domain component
Γö£ΓöÇΓöÇ story_io_increment.py          # Increment (release) component
Γö£ΓöÇΓöÇ story_io_position.py           # Position and Boundary types
Γöé
Γö£ΓöÇΓöÇ story_io_renderer.py           # DrawIO rendering engine wrapper
Γö£ΓöÇΓöÇ story_io_synchronizer.py       # DrawIO synchronization engine wrapper
Γö£ΓöÇΓöÇ story_io_cli.py                # Command-line interface
Γö£ΓöÇΓöÇ story_io_mcp_server.py         # MCP server with tools
Γöé
Γö£ΓöÇΓöÇ story_map_drawio_synchronizer.py  # Legacy synchronizer (internal)
Γöé
Γö£ΓöÇΓöÇ cli/                           # PowerShell wrapper and examples
Γöé   Γö£ΓöÇΓöÇ story-io.ps1               # Main PowerShell CLI wrapper
Γöé   Γö£ΓöÇΓöÇ render-examples.ps1        # Rendering examples
Γöé   Γö£ΓöÇΓöÇ sync-examples.ps1          # Synchronization examples
Γöé   Γö£ΓöÇΓöÇ search-examples.ps1        # Search examples
Γöé   Γö£ΓöÇΓöÇ merge-examples.ps1         # Merge examples
Γöé   ΓööΓöÇΓöÇ complete-workflow.ps1      # End-to-end workflow example
Γöé
Γö£ΓöÇΓöÇ examples/                      # Python code examples
Γöé   Γö£ΓöÇΓöÇ render_example.py          # Basic rendering
Γöé   Γö£ΓöÇΓöÇ load_and_render_example.py # Load and render pattern
Γöé   Γö£ΓöÇΓöÇ add_user_example.py        # Add user to story
Γöé   ΓööΓöÇΓöÇ *.ps1                      # PowerShell examples (duplicates of cli/)
Γöé
Γö£ΓöÇΓöÇ test/                          # Unit tests (BDD with Mamba)
Γöé   Γö£ΓöÇΓöÇ test_story_io_component.py # Base component tests
Γöé   Γö£ΓöÇΓöÇ test_story_io_position.py  # Position/Boundary tests
Γöé   Γö£ΓöÇΓöÇ test_story_io_epic.py      # Epic tests
Γöé   Γö£ΓöÇΓöÇ test_story_io_feature.py   # Feature tests
Γöé   Γö£ΓöÇΓöÇ test_story_io_story.py     # Story tests
Γöé   Γö£ΓöÇΓöÇ test_story_io_user.py      # User tests
Γöé   ΓööΓöÇΓöÇ test_story_io_increment.py # Increment tests
Γöé
ΓööΓöÇΓöÇ acceptance/                    # Acceptance tests with real files
    Γö£ΓöÇΓöÇ test_acceptance.py         # Acceptance test runner
    ΓööΓöÇΓöÇ scenarios/                 # Test scenarios
        Γö£ΓöÇΓöÇ simple/                # Minimal story graph
        Γö£ΓöÇΓöÇ complex/               # Complex multi-epic graph
        Γö£ΓöÇΓöÇ single_epic/           # Single epic scenario
        Γö£ΓöÇΓöÇ multiple_users/        # Multiple user scenario
        ΓööΓöÇΓöÇ with_increments/       # Increments scenario
```
...
- **Sync**: DrawIO ΓåÆ extracts to JSON graph ΓåÆ can save JSON or render back to DrawIO
- **Merge**: JSON graph + JSON graph ΓåÆ merged JSON graph (works with JSON files)
- **Render**: JSON graph ΓåÆ DrawIO (we build DrawIO from graphs)
...
- Sync: DrawIO ΓåÆ extracts to JSON graph ΓåÆ can save JSON or render back to DrawIO
- Merge: JSON graph + JSON graph ΓåÆ merged JSON graph
- Render: JSON graph ΓåÆ DrawIO (we build DrawIO from graphs)
...
**Render Outline (Epics ΓåÆ Features ΓåÆ Stories):**
...
Γöé   Γöé   Γö£ΓöÇΓöÇ Story
Γöé   Γöé   Γöé   ΓööΓöÇΓöÇ User (bidirectional relationship)
Γöé   Γöé   ΓööΓöÇΓöÇ Story...
Γöé   ΓööΓöÇΓöÇ Feature...
ΓööΓöÇΓöÇ Increment
    Γö£ΓöÇΓöÇ Epic (references)
    Γö£ΓöÇΓöÇ Feature (references)
    ΓööΓöÇΓöÇ Story (references by name)
```
...
- Test round-trip: DrawIO ΓåÆ modify ΓåÆ DrawIO (preserve layout)
...
Comparison:
  Epics: 1 ΓåÆ 1 (match)
  Features: 1 ΓåÆ 1 (match)
  Stories: 1 ΓåÆ 1 (match)
