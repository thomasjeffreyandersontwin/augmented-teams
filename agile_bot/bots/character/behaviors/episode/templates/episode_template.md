...
- `{episode-title}` - Episode title (from parameter or default: "Episode {date}")
- `{episode-description}` - Episode description (from parameter or empty)
- `{episode-date}` - Current date (automatically generated)
- `{character-name}` - Character name (from parameter or default from CharacterChatAgent)

## Usage

This template is used when creating new episodes via `/episode start` command. The template is copied to `behaviors/character/profiles/{character-name}/episodes/current-episode-in-progress.md` with all placeholders filled in.
