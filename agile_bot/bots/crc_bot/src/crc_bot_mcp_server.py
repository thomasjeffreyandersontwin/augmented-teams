"""
Crc Bot MCP Server Entry Point

Runnable MCP server for crc_bot using FastMCP with statically generated tools.
"""
...
def main():
    """Main entry point for crc_bot MCP server.
    ...
    bot = Bot(bot_name='crc_bot', bot_directory=bot_directory, config_path=bot_directory / 'bot_config.json')
    
    server_name = 'crc_bot'
    mcp_server = FastMCP(server_name)
    
        @mcp_server.tool(name='tool', description='Bot tool for crc_bot - routes to current behavior and action.')
    ...
    @mcp_server.tool(name='close_current_action', description='Close current action tool for crc_bot - marks current action complete and transitions to next')
    ...
    @mcp_server.tool(name='confirm_out_of_order', description='Confirm out-of-order behavior execution for crc_bot - MUST be called explicitly by HUMAN USER, NOT by AI assistant. AI must ask user to call this tool, never call it directly.')
    ...
    @mcp_server.tool(name='restart_server', description='Restart MCP server for crc_bot - terminates processes, clears cache, and restarts to load code changes')
    async def restart_server(parameters: dict=None):
        try:
            ...
            result = restart_mcp_server(workspace_root=workspace_root, bot_name='crc_bot', bot_location=bot_location)
            return result
        except Exception as e:
        ...
        @mcp_server.tool(name='domain', description='domain behavior for crc_bot. Accepts optional action parameter and parameters dict.
Trigger patterns: shape.*crc, model.*crc, create.*crc.*cards, define.*crc, discover.*responsibilities')
    async def domain_tool(action: str=None, parameters: dict=None):
        behavior_obj = bot.behaviors.find_by_name('domain')
        if action:
            action_obj = behavior_obj.actions.find_by_name(action)
            result_data = action_obj.execute(parameters or {})
            return {'status': 'completed', 'behavior': 'domain', 'action': action, 'data': result_data}
        ...
        return {'status': 'completed', 'behavior': 'domain', 'action': current_action.action_name, 'data': result_data}

    @mcp_server.tool(name='design', description='design behavior for crc_bot. Accepts optional action parameter and parameters dict.
Trigger patterns: design.*architecture, apply.*oop, design.*patterns, refine.*design, object.*responsibilities')
    async def design_tool(action: str=None, parameters: dict=None):
        behavior_obj = bot.behaviors.find_by_name('design')
        if action:
            action_obj = behavior_obj.actions.find_by_name(action)
            result_data = action_obj.execute(parameters or {})
            return {'status': 'completed', 'behavior': 'design', 'action': action, 'data': result_data}
        behavior_obj.actions.load_state()
        current_action = behavior_obj.actions.current
        result_data = current_action.execute(parameters or {})
        return {'status': 'completed', 'behavior': 'design', 'action': current_action.action_name, 'data': result_data}

    @mcp_server.tool(name='walkthrough', description='walkthrough behavior for crc_bot. Accepts optional action parameter and parameters dict.
Trigger patterns: walkthrough.*domain, trace.*object.*flow, realize.*scenario, test.*domain.*model, validate.*model.*through.*scenarios')
    async def walkthrough_tool(action: str=None, parameters: dict=None):
        behavior_obj = bot.behaviors.find_by_name('walkthrough')
        if action:
            action_obj = behavior_obj.actions.find_by_name(action)
            result_data = action_obj.execute(parameters or {})
            return {'status': 'completed', 'behavior': 'walkthrough', 'action': action, 'data': result_data}
        behavior_obj.actions.load_state()
        current_action = behavior_obj.actions.current
        result_data = current_action.execute(parameters or {})
        return {'status': 'completed', 'behavior': 'walkthrough', 'action': current_action.action_name, 'data': result_data}
