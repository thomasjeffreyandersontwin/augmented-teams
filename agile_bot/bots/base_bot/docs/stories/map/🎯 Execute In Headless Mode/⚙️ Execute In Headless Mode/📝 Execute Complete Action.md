...
1. **CLI executes complete action workflow in headless mode:**
   ...
   - AND CLI tracks operations_executed list with "instructions", "confirm"
...
2. **CLI handles auto_confirm for configured actions:**
   - WHEN action configuration specifies auto_confirm=true
   - THEN CLI automatically runs confirm after instructions complete
   - AND CLI does not wait for manual confirm command

3. **CLI handles block during action workflow:**
...
4. **CLI recovers from AI getting stuck during action workflow:**
...
### Scenario: Execute action with auto_confirm enabled

**Steps:**
- Given AI has written headless-context.md with user intent and chat history
- And headless mode is configured with API key
- And user wants to execute shape.clarify action
- And shape.clarify action configuration specifies auto_confirm=true
- When human invokes CLI with --headless flag and target shape.clarify
- Then CLI executes instructions operation
- And CLI detects instructions completion (done=true)
- And CLI automatically executes confirm operation (due to auto_confirm=true)
...
- And confirm operation blocks waiting for clarification (blocked=true)
- Then CLI detects blocked state (blocked=true) during confirm
- And CLI stops action workflow execution
- And CLI sets blocked_operation to "confirm"
- And CLI sets operations_status: {"instructions": "completed", "confirm": "blocked"}
- And CLI reports which operation blocked (confirm)
- And CLI displays block reason from API response
...
- And AI is stuck during confirm operation (reached MAX_LOOPS limit of 50)
