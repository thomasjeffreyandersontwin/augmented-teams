...
Inject Template Instructions functionality for the mob minion system.
...
### Scenario: Render output loads template files from render JSON (happy_path)
...
### Scenario: Render output injects template content into instructions (happy_path)
...
### Scenario: Render output handles missing template files gracefully (happy_path)
