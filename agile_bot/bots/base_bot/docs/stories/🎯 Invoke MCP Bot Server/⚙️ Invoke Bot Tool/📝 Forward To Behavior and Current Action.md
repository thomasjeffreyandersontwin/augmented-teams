...
Bot tool forwards invocations to current behavior and current action based on workflow state so that users resume at the precise point they left off with both behavior and action determined from saved state.

## Acceptance Criteria

### Behavioral Acceptance Criteria

- **When** Bot tool receives invocation, **then** Bot tool loads Workflow State from persistent storage
- **When** Bot tool loads Workflow State, **then** Bot tool extracts current_behavior and current_action from state
- **When** Bot tool extracts current_behavior and current_action, **then** Bot tool forwards call to Behavior[current_behavior].Action[current_action]
- **When** Bot tool forwards to current behavior and action, **then** Forwarding ensures user resumes at exact point they left off
- **When** workflow state file doesn't exist, **then** Bot tool defaults to first behavior/first action
...
Then Bot tool loads workflow state from project_area/workflow state
And Bot tool extracts current_behavior='story_bot.discovery'
And Bot tool extracts current_action='story_bot.discovery.build_knowledge'
And Bot tool forwards call to discovery behavior with parameter action='build_knowledge'
And Bot tool does NOT forward to first behavior or first action
And User resumes at exact behavior and action where they left off
```
