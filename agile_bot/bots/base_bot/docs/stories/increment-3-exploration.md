...
- (AC) AND Router loads workflow_state.json from {project_area}/workflow_state.json
...
- (AC) AND Activity entry appended to {project_area}/activity_log.json
...
- (AC) AND Workflow automatically proceeds to next action per action_config.json
...
- (AC) AND Workflow automatically proceeds to render_output (auto_progress: true in action_config.json)
...
- (AC) AND validate_rules is terminal action (next_action: null in action_config.json)
...
### Core Concepts:
- **Action Configuration**: Each action folder contains action_config.json specifying: name, workflow (true/false), order, next_action, and optional auto_progress flag
- **Workflow Action**: Action with workflow: true in action_config.json. Has order number, next_action defined, triggers automatic transitions
- **Independent Action**: Action with workflow: false in action_config.json (e.g., correct_bot). No order, no next_action, invoked independently by user, doesn't trigger automatic transitions
...
- **Action Discovery**: Generator scans base_actions/ folder and reads action_config.json from each folder to discover workflow vs independent actions
