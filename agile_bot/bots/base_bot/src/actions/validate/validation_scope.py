...
class ValidationScope(ActionScope):
    ...
    def __init__(self, parameters: Dict[str, Any], bot_paths: Optional[BotPaths]=None, behavior_name: Optional[str]=None):
        ...
        if not exclude_patterns:
            exclude_patterns = parameters.get('exclude', [])
    ...
    def _auto_discover_if_needed(self, key, files_list):
        ...
        if not files_list and (not has_any_explicit_files) and (not has_files_scope) and self._bot_paths:
    ...
    def _handle_behavior_specific_files(self, all_files_dict):
        ...
        has_files_scope = (self._parameters.get('scope', {}).get('type') == 'files' if isinstance(self._parameters.get('scope'), dict) else False)
        if not has_explicit_files and (not has_any_explicit_params) and (not has_files_scope):
    ...
    def _handle_general_file_discovery(self, all_files_dict):
        ...
        has_files_scope = (self._parameters.get('scope', {}).get('type') == 'files' if isinstance(self._parameters.get('scope'), dict) else False)
        file_keys = {'test', 'src'}
        if not has_any_explicit_params and not has_files_scope:
