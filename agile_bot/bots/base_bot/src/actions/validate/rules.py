...
from agile_bot.bots.base_bot.src.actions.validate.rule_loader import RuleLoader
from agile_bot.bots.base_bot.src.actions.validate.rule_filter import RuleFilter
...
class ValidationContext:
    ...
    def from_action_context(cls, behavior, context: 'ValidateActionContext', callbacks: Optional[ValidationCallbacks] = None) -> 'ValidationContext':
        """Create ValidationContext from typed ValidateActionContext."""
        ...
        # Extract values from typed context
    ...
    def from_parameters(cls, parameters: Dict[str, Any], behavior, bot_paths, callbacks: Optional[ValidationCallbacks] = None) -> 'ValidationContext':
        """Create ValidationContext from parameters dict (converts to typed context internally).
        
        Args:
            parameters: Dict of validation parameters
            behavior: Either a Behavior object or a behavior name string
            bot_paths: BotPaths instance
            callbacks: Optional validation callbacks
        """
        from agile_bot.bots.base_bot.src.actions.action_context import ValidateActionContext, ScopeConfig, ScopeType
        from agile_bot.bots.base_bot.src.bot.behavior import Behavior
        
        # If behavior is a string, create Behavior object
...
class Rules:
    ...
    def _execute_scanner(self, rule, rule_result: dict, knowledge_graph: Dict, files: Dict, scanner_path: str, on_scanner_start, on_file_scanned, skip_cross_file: bool, logger, changed_files: Optional[Dict]=None, all_files: Optional[Dict]=None) -> str:
        ...
        try:
            scanner_results = rule.scan(knowledge_graph, all_files or files, on_file_scanned=on_file_scanned, skip_cross_file=skip_cross_file, changed_files=changed_files)
    ...
    def _process_rule(self, rule, rule_result: dict, knowledge_graph: Dict, files: Dict, on_scanner_start, on_file_scanned, skip_cross_file: bool, logger, changed_files: Optional[Dict]=None, all_files: Optional[Dict]=None) -> str:
        ...
        return self._execute_scanner(rule, rule_result, knowledge_graph, files, scanner_path, on_scanner_start, on_file_scanned, skip_cross_file, logger, changed_files, all_files)

    def validate(self, validation_context_or_knowledge_graph, files: Optional[Dict[str, List[Path]]]=None, callbacks: Optional[ValidationCallbacks]=None, skiprule: Optional[List[str]]=None, exclude: Optional[List[str]]=None) -> List[Dict[str, Any]]:
        if isinstance(validation_context_or_knowledge_graph, ValidationContext):
            return self._execute_validation(validation_context_or_knowledge_graph)
        return self._execute_validation(self._create_legacy_context(validation_context_or_knowledge_graph, files, callbacks, skiprule, exclude))
    ...
    def _create_legacy_context(self, knowledge_graph: Dict, files: Optional[Dict], callbacks: Optional[ValidationCallbacks], skiprule: Optional[List[str]], exclude: Optional[List[str]]) -> ValidationContext:
        return ValidationContext(knowledge_graph=knowledge_graph, files=files or {}, callbacks=callbacks or ValidationCallbacks(), skiprule=skiprule or [], exclude=exclude or [], skip_cross_file=True, force_full=False, behavior=self.behavior, bot_paths=getattr(self, 'bot_paths', None), working_dir=Path.cwd())
    ...
    def _execute_validation(self, context: ValidationContext) -> List[Dict[str, Any]]:
        ...
        for idx, rule in enumerate(rules_list, 1):
            ...
            try:
                ...
                status_line = self._process_rule(rule, rule_result, context.knowledge_graph, files, on_scanner_start, on_file_scanned, context.skip_cross_file, logger, changed_files, all_files)
