...
class ValidationContext:
    ...
    @classmethod
    def from_parameters(cls, behavior, parameters: Dict[str, Any], callbacks: Optional[ValidationCallbacks]=None) -> 'ValidationContext':
        ...
        return cls(knowledge_graph=story_graph.content, files=files, callbacks=callbacks or ValidationCallbacks(), skiprule=parameters.get('skiprule', []), exclude=parameters.get('exclude', []), skip_cross_file=parameters.get('skip_cross_file', False), behavior=behavior, bot_paths=behavior.bot_paths, working_dir=behavior.bot_paths.workspace_directory)
...
class Rules:
    ...
    def _execute_scanner(self, rule, rule_result: dict, knowledge_graph: Dict, files: Dict, scanner_path: str, on_scanner_start, on_file_scanned, skip_cross_file: bool, logger) -> str:
        ...
        try:
            scanner_results = rule.scan(knowledge_graph, files, on_file_scanned=on_file_scanned, skip_cross_file=skip_cross_file)
    ...
    def _process_rule(self, rule, rule_result: dict, knowledge_graph: Dict, files: Dict, on_scanner_start, on_file_scanned, skip_cross_file: bool, logger) -> str:
        ...
        return self._execute_scanner(rule, rule_result, knowledge_graph, files, scanner_path, on_scanner_start, on_file_scanned, skip_cross_file, logger)
    ...
    def _create_legacy_context(self, knowledge_graph: Dict, files: Optional[Dict], callbacks: Optional[ValidationCallbacks], skiprule: Optional[List[str]], exclude: Optional[List[str]]) -> ValidationContext:
        return ValidationContext(knowledge_graph=knowledge_graph, files=files or {}, callbacks=callbacks or ValidationCallbacks(), skiprule=skiprule or [], exclude=exclude or [], skip_cross_file=False, behavior=self.behavior, bot_paths=getattr(self, 'bot_paths', None), working_dir=Path.cwd())
    ...
    def _execute_validation(self, context: ValidationContext) -> List[Dict[str, Any]]:
        ...
        for idx, rule in enumerate(rules_list, 1):
            ...
            try:
                ...
                status_line = self._process_rule(rule, rule_result, context.knowledge_graph, files, on_scanner_start, on_file_scanned, context.skip_cross_file, logger)
