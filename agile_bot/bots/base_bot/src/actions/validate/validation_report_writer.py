...
class StreamingValidationReportWriter:
    ...
    def _extract_violation_fields(self, violation: Dict[str, Any]) -> tuple:
    ...
    def _write_single_violation(self, violation: Dict[str, Any]) -> None:
    ...
    def _check_for_errors(self, rule_result):
        ...
        file_by_file_errors = any((v.get('severity') == 'error' for v in scanner_results.get('file_by_file', {}).get('violations', [])))
        cross_file_errors = any((v.get('severity') == 'error' for v in scanner_results.get('cross_file', {}).get('violations', [])))
...
class ValidationReportWriter:
    ...
    def _check_violations_in_key(self, violations_data: Dict[str, Any]) -> tuple:
        """Check for errors and warnings in a violations data structure."""
        has_errors = False
        has_warnings = False
    ...
    def _format_violation_line(self, violation: Dict[str, Any]) -> List[str]:
        """Format a single violation line for display."""
