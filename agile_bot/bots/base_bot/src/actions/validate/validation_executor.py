...
class ValidationExecutor:
    ...
    def execute_synchronous(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        logger = logging.getLogger(__name__)
        try:
            validation_context = ValidationContext.from_parameters(behavior=self.behavior, parameters=parameters)
            ...
            self._log_error(e, parameters, logger)
            raise
    ...
    def _log_error(self, e: Exception, parameters: Dict[str, Any], logger) -> None:
        logger.error(f'Error in synchronous validation: {e}')
        logger.error(f'Error type: {type(e).__name__}, Parameters: {parameters}')
