...
class MCPCodeVisitor(Visitor):
    ...
    def __init__(self, workspace_root: Path, bot_location: Path, bot_name: str, behaviors: List[str], data_collector: ActionDataCollector):
...
if __name__ == '__main__':
    ...
    def _generate_bot_tool_code(self) -> str:
        return f'''    @mcp_server.tool(name='tool', description='Bot tool for {self.bot_name} - routes to current behavior and action.')
    ...
    def _generate_close_action_code(self) -> str:
        return f'''    @mcp_server.tool(name='close_current_action', description='Close current action tool for {self.bot_name} - marks current action complete and transitions to next')
    ...
    def _generate_confirm_out_of_order_code(self) -> str:
        return f'''    @mcp_server.tool(name='confirm_out_of_order', description='Confirm out-of-order behavior execution for {self.bot_name} - MUST be called explicitly by HUMAN USER, NOT by AI assistant. AI must ask user to call this tool, never call it directly.')
    ...
    def _generate_restart_server_code(self) -> str:
        return f'''    @mcp_server.tool(name='restart_server', description='Restart MCP server for {self.bot_name} - terminates processes, clears cache, and restarts to load code changes')
