...
def get_base_bot_dir(repo_root: Path) -> Path:
    """Get test_base_bot directory path for tests."""
...
def create_common_rules(repo_root: Path, rules: list) -> Path:
    """Create common rules in test_base_bot directory."""
...
def create_base_action_instructions(bot_directory: Path, action: str) -> Path:
    """Create base action config for specific action in agile_bot/base_actions.
...
def _is_production_story_bot_path(path: Path) -> bool:
    ...
    try:
        ...
        # Get the repo root (agile_bot/test/test_helpers.py -> repo root is 3 levels up)
        # __file__ = agile_bot/test/test_helpers.py
        # .parent = agile_bot/test
        # .parent.parent = agile_bot
...
def create_actions_workflow_json(bot_directory: Path, behavior_name: str, actions: list = None, order: int = None) -> Path:
    ...
    Raises:
        RuntimeError: If attempting to write to production story_bot behavior.json files
    """
    import json
        ...
        actions = [
            {
                ...
                "instructions": [
                    f"Follow agile_bot/base_actions/clarify/action_config.json",
            ...
            },
            {
                ...
                "instructions": [
                    f"Follow agile_bot/base_actions/strategy/action_config.json",
            ...
            },
            {
                ...
                "next_action": "render",
                "instructions": [
                    f"Follow agile_bot/base_actions/validate/action_config.json",
            ...
            },
            {
                ...
                "order": 4,
                "instructions": [
                    f"Follow agile_bot/base_actions/render/action_config.json",
    ...
    # Note: actions_workflow.json is deprecated - all actions_workflow should be in behavior.json
    # We no longer create actions_workflow.json files as they've been removed from production
    
    
    return behavior_file

def create_base_actions_structure(bot_directory: Path):
    """Create base actions directory structure in agile_bot/base_actions.
    
    Tests should use the shared agile_bot/base_actions directory and must not
    generate action configs inside a bot folder. We simply ensure the shared
    directory exists; no files are created.
    """
    base_actions_dir = get_test_base_actions_dir(bot_directory)
    if not base_actions_dir.exists():
        raise RuntimeError(f"Shared base_actions directory not found at {base_actions_dir}. Tests must use existing base actions in agile_bot/base_actions.")
...
# Import from: from agile_bot.test.test_execute_behavior_actions import ...
...
def setup_bot_behavior_action(
    workspace_directory: Path,
    bot_name: str = 'story_bot',
    behavior: str = None,
    current_action: str = None,
    completed_actions: list = None,
    behaviors: list = None,
    create_state_file: bool = True,
    navigate_to_behavior: bool = True,
    navigate_to_action: bool = True,
    use_actual_bot_directory: bool = True
) -> tuple:
    """Unified setup function for bot, behavior, and action instances.
    
    This function consolidates all bot/behavior/action setup patterns across test files.
    It handles:
    - Bot instance creation (using actual story_bot directory by default)
    - Workspace directory setup
    - State file creation (behavior_action_state.json)
    - Behavior navigation
    - Action navigation
    ...
    Args:
        ...
        behavior: Behavior name to navigate to (if None, uses first behavior)
        current_action: Current action name (if None, uses first action)
        completed_actions: List of completed action entries (format: [{'action_state': 'bot.behavior.action', 'timestamp': '...'}])
        behaviors: List of behavior names (if None, uses behaviors from actual bot)
        create_state_file: If True, creates behavior_action_state.json file
        navigate_to_behavior: If True, navigates to specified behavior
        navigate_to_action: If True, navigates to specified action (requires navigate_to_behavior=True)
        use_actual_bot_directory: If True, uses actual story_bot directory; if False, uses workspace_directory/bot_name
    
    Returns:
        tuple: (bot, state_file_path) where:
            - bot: Bot instance
            - state_file_path: Path to behavior_action_state.json (or None if create_state_file=False)
    
    Example:
        # Basic setup with default behavior and action
        bot, state_file = setup_bot_behavior_action(workspace_dir, bot_name='story_bot')
        
        # Setup with specific behavior and action
        bot, state_file = setup_bot_behavior_action(
            workspace_dir,
            bot_name='story_bot',
            behavior='shape',
            current_action='build',
            completed_actions=[{'action_state': 'story_bot.shape.clarify', 'timestamp': '2024-01-01T00:00:00'}]
        )
        
        # Setup without state file (for tests that mock state)
        bot, _ = setup_bot_behavior_action(
            workspace_dir,
            create_state_file=False,
            navigate_to_behavior=False
        )
    """
    import json
    import os
    ...
    if use_actual_bot_directory:
        # Use actual story_bot directory (has all behaviors and configs)
        repo_root = Path(__file__).resolve().parent.parent.parent
        bot_directory = repo_root / 'agile_bot' / 'bots' / bot_name
        if not bot_directory.exists():
            raise FileNotFoundError(f"Bot directory not found: {bot_directory}")
    else:
        # Use workspace-relative bot directory (for tests that need isolated setup)
        bot_directory = workspace_directory.parent / 'agile_bot' / 'bots' / bot_name
    ...
    if create_state_file:
        state_file_path = workspace_directory / 'behavior_action_state.json'
        state_data = {
            'current_behavior': '',
            'current_action': '',
        ...
        # Set current_behavior and current_action if provided
        if behavior:
            state_data['current_behavior'] = f'{bot_name}.{behavior}'
            if current_action and current_action.strip():
                state_data['current_action'] = f'{bot_name}.{behavior}.{current_action}'
        
        state_file_path.write_text(json.dumps(state_data, indent=2), encoding='utf-8')
    ...
    # Navigate to behavior if requested
    if navigate_to_behavior and behavior:
        if bot.behaviors.current is None or bot.behaviors.current.name != behavior:
            bot.behaviors.navigate_to(behavior)
            # CRITICAL: Ensure behavior.bot_name matches bot.bot_name
            # The behavior's bot_name comes from bot_paths.bot_directory.name, which might differ from bot.bot_name
            # We need the state file to match what load_state() will check against
            target_behavior = bot.behaviors.find_by_name(behavior)
            if target_behavior:
                # Use the behavior's actual bot_name for the state file to ensure match
                actual_bot_name = target_behavior.bot_name
                # Update state file if it was created with wrong bot_name
                if create_state_file and state_file_path and state_file_path.exists():
                    import json
                    state_data = json.loads(state_file_path.read_text(encoding='utf-8'))
                    # Update state file with correct bot_name
                    state_data['current_behavior'] = f'{actual_bot_name}.{behavior}'
                    if current_action:
                        state_data['current_action'] = f'{actual_bot_name}.{behavior}.{current_action}'
                    # Update completed_actions with correct bot_name
                    if state_data.get('completed_actions'):
                        updated_completed = []
                        for entry in state_data['completed_actions']:
                            action_state = entry.get('action_state', '')
                            if action_state.startswith(f'{bot_name}.'):
                                # Replace bot_name in action_state
                                action_state = action_state.replace(f'{bot_name}.', f'{actual_bot_name}.', 1)
                                entry['action_state'] = action_state
                            updated_completed.append(entry)
                        state_data['completed_actions'] = updated_completed
                    state_file_path.write_text(json.dumps(state_data, indent=2), encoding='utf-8')
                
                # Ensure behavior.bot_name matches bot.bot_name for consistency
                target_behavior.bot_name = bot.bot_name
                # Reload state after navigation
                target_behavior.actions.load_state()
    
    # Navigate to action if requested
    if navigate_to_action and current_action and bot.behaviors.current:
        actions = bot.behaviors.current.actions
        if actions.current is None or actions.current.action_name != current_action:
            # Use out_of_order=True to bypass file I/O during tests
            actions.navigate_to(current_action, out_of_order=True)
    elif navigate_to_action and bot.behaviors.current:
        # No current_action specified, ensure first action is selected
        actions = bot.behaviors.current.actions
        if actions._actions and actions._current_index is None:
            actions._current_index = 0
...
def given_environment_setup(bot_dir: Path, workspace_dir: Path, behaviors: list = None, setup_type: str = 'standard', bot_name: str = 'story_bot', **kwargs) -> Path:
    ...
    """
    from agile_bot.test.test_helpers import create_base_actions_structure, create_bot_config_file
    from agile_bot.test.test_helpers import create_actions_workflow_json
    
    # If using production story_bot, skip file creation - use existing files
    if _is_production_story_bot_path(bot_dir / 'bot_config.json'):
        bootstrap_env(bot_dir, workspace_dir)
        return bot_dir / 'bot_config.json'
    ...
    if not _is_production_story_bot_path(bot_dir / 'bot_config.json'):
        for i, behavior_name in enumerate(behaviors, start=1):
            if setup_type == 'final_action' and behavior_name == 'shape':
                ...
                from agile_bot.test.test_invoke_bot_directly import given_workflow_config
    ...
    if setup_type == 'resume':
        ...
        from agile_bot.test.test_invoke_bot_directly import given_completed_action
...
def given_workflow_config(bot_directory: Path, behaviors: list = None, actions: list = None, final_action: str = None) -> None:
    ...
    from agile_bot.test.test_helpers import create_actions_workflow_json, get_test_base_actions_dir
    from agile_bot.test.domain.test_execute_behavior_actions import create_minimal_guardrails_files
...
def given_action_initialized(action_type, bot_directory, bot_name='story_bot', behavior='exploration', **kwargs):
    ...
    from pathlib import Path
    from agile_bot.src.bot.bot_paths import BotPaths
    from agile_bot.test.test_helpers import create_actions_workflow_json, bootstrap_env
    from agile_bot.test.domain.test_execute_behavior_actions import create_minimal_guardrails_files
    from agile_bot.test.test_invoke_mcp import given_base_actions_setup
...
def when_scanner_scans(scanner_instance, bad_example, rule_obj, scanner_type='auto'):
    ...
    """
    from pathlib import Path
    from agile_bot.src.scanners.test_scanner import TestScanner
    from agile_bot.src.scanners.code_scanner import CodeScanner
...
def then_instructions_have_structure(instructions, structure='validation_rules'):
    ...
    if structure == 'validation_rules':
        ...
        from agile_bot.test.test_validate_knowledge_and_content_against_rules import validate_violation_structure
...
def then_action_instructions_match(action, knowledge_graph=None, test_files=None):
    ...
    """
    from agile_bot.src.actions.rules.rules import ValidationContext, ValidationCallbacks
...
def bot_directory(tmp_path):
    """Fixture: Bot directory.
    
    Creates a temporary copy of the real story_bot so tests can read the
    defaults without writing to production files.
    """
    from pathlib import Path
    repo_root = Path(__file__).resolve().parent.parent.parent
    source_bot_dir = repo_root / 'agile_bot' / 'bots' / 'story_bot'
    temp_bot_dir = tmp_path / 'story_bot'
...
def create_behavior_folder_with_json(bot_dir: Path, folder_name: str) -> Path:
    ...
    # Ensure shared base_actions exist in agile_bot/base_actions (not under bot_dir)
    from agile_bot.test.test_helpers import create_base_actions_structure
    create_base_actions_structure(bot_dir)
...
class TestFindBehaviorFolder:
    ...
    def test_handles_tests_folder_with_prefix(self, bot_directory):
        """
        SCENARIO: Handles Tests Folder With Prefix
        GIVEN: Behavior folder exists as 'tests'
        WHEN: find_behavior_folder is called with behavior name (tests)
        THEN: Returns path to 'tests'
        """
        # Given
        bot_name = 'story_bot'
        behavior_folder = create_behavior_folder(bot_directory, 'tests')
        
        # When: Behavior folders are directly named
        found_folder = bot_directory / 'behaviors' / 'tests'
...
def given_action_setup(action_type, bot_directory, behavior):
    ...
    from pathlib import Path
    from agile_bot.test.test_helpers import create_base_actions_structure
    from agile_bot.test.test_helpers import bootstrap_env
    from agile_bot.src.bot.bot_paths import BotPaths
    from agile_bot.src.bot.behavior import Behavior
...
def when_action_executes(action_type, bot_directory, behavior, **execution_params):
    ...
    workspace_directory = execution_params.get('workspace_directory')
    ...
    # If a workspace directory is provided, work on a temporary bot copy there to avoid touching production files
    if workspace_directory:
        import shutil
        ws_path = Path(workspace_directory).resolve()
        bot_dir_path = Path(bot_directory).resolve()
        # If we're already using a temp bot under the workspace, reuse it
        if bot_dir_path.is_relative_to(ws_path):
            temp_bot_dir = bot_dir_path
        else:
            temp_bot_dir = ws_path / 'temp_bot'
            shutil.copytree(bot_dir_path, temp_bot_dir, dirs_exist_ok=True)
        bot_directory = temp_bot_dir
    ...
    if action_obj is None:
        ...
        if action_type == 'build':
            ...
            bot_paths = BotPath(workspace_path=workspace_directory, bot_directory=bot_directory)
            behavior_obj = Behavior(name=behavior, bot_paths=bot_paths, bot_instance=None)
