...
def verify_action_tracks_start(bot_dir: Path, workspace_dir: Path, action_class, action_name: str, 
    ...
    behavior_obj.bot_paths = MockBotPath(bot_dir, workspace_dir)
    behavior_obj.bot = None
    
    # Create activity tracker - ActivityTracker needs BotPath, not Path
    from agile_bot.src.bot_path import BotPath
    bot_paths = BotPath(workspace_path=workspace_dir, bot_directory=bot_dir)
...
def create_minimal_guardrails_files(bot_dir: Path, behavior_name: str, bot_name: str = 'story_bot'):
