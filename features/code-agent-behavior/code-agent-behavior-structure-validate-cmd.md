...
4. **Relationship sections** (documentation that links rules, commands, and code):
   ...
   * Commands must reference:
     - `**Steps:**` — Sequential actions to execute the behavior (REQUIRED, must specify who/what performs each: AI agent, user, code function, or MCP tool)
...
4. **Steps format validation:**
   * Each step must specify who/what performs it (e.g., "The AI agent...", "The code function...", "The user is prompted...", "The MCP tool...")
   * Steps without performers are flagged as violations
6. Files are documented with proper headers
7. Related files share consistent naming prefixes
8. Rules must start with `**When** <event> condition,`
9. Index files (`behavior-index.json`) are excluded from validation
...
* All behavior files must be in `features/<feature>/code-agent-behaviors/` (organized in Behavior Features - collections of tightly-knit, strongly related behaviors)
...
* Index files (`code-agent-behavior-index.json`) are stored in `features/<feature>/code-agent-behavior-index.json`
* **Behavior Features**: Keep behavior files belonging to the same feature together in one directory (e.g., `features/code-agent-behavior/code-agent-behaviors/`). Never scatter behaviors into separate areas or break them up by component type.
