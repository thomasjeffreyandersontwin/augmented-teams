def behavior_index(feature=None):
    ...
    Scans features/*/cursor/ folders for behavior files and maintains:
    - Local indexes: features/<feature>/behavior-index.json
    - Global index: .cursor/behavior-index.json
    
    Rules:
    1. Detect additions, updates, or deletions in features/*/cursor/
    ...
    for cursor_path in features:
        ...
        for file in cursor_path.rglob("*"):
            ...
            if file.suffix == ".mdc":
                ...
                deployed_location = ".cursor/commands/"
                deployed_file = f".cursor/commands/{file.name}"
            elif file.suffix == ".py":
                deployed_location = "commands/"
                deployed_file = f"commands/{file.name}"
            ...
            try:
                ...
                behavior_entry = {
                    ...
                    "file": file.name,
                    "type": file.suffix,
                    "path": str(file.relative_to(src_root.parent)),
    ...
    for cursor_path in features:
        ...
        try:
            ...
            print(f"✅ Local index updated: {local_index.relative_to(src_root.parent)}")
        except Exception as e:
    ...
    if skipped_files:
        ...
        for file, reason in skipped_files:
            print(f"  {file.relative_to(src_root)} ({reason})")
...
def sync_purposes_from_global(feature=None):
    ...
    features_to_update = [feature] if feature else [f.name for f in src_root.iterdir() if f.is_dir()]
    updated_count = 0
    
    for feature_name in features_to_update:
        local_index = src_root / feature_name / "behavior-index.json"
        ...
        try:
            ...
            if updated:
                ...
                print(f"✅ Updated purposes in {local_index.relative_to(src_root.parent)}")
                updated_count += 1
