...
**Always:**
- ✅ Detects additions, updates, or deletions in `features/*/cursor/`
- ✅ Records feature name, file type, path, and modification timestamp
- ✅ Updates both local (`features/<feature>/behavior-index.json`) and global (`.cursor/behavior-index.json`) indexes
...
```bash
python features/cursor-behavior/cursor/behavior-index-cmd.py sync-purposes
```
...
features/<feature>/
  ...
  └── behavior-index.json  ← Local index (not in cursor/)
```
