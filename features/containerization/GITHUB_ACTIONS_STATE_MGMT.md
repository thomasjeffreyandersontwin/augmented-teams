...
- name: Check pipeline state
  id: state
  run: |
    if [ -f "features/${{ env.FEATURE }}/.deployment-state.json" ]; then
      echo "status=$(jq -r '.status' features/${{ env.FEATURE }}/.deployment-state.json)" >> $GITHUB_OUTPUT
...
- name: Provision if not completed
  ...
  run: |
    python features/${{ env.FEATURE }}/config/provision-service.py AZURE
```
...
jobs:
  deploy-features:
    ...
    - name: Check state
      run: |
        STATE_FILE="features/${{ matrix.feature }}/.deployment-state.json"
