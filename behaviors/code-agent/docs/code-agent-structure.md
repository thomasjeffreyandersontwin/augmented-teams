...
- ✅ Keep each behavior in its own feature folder under `behaviors/<feature>/` (mark with `code-agent-behavior.json`)
...
Index files (`code-agent-index.json`) are stored in `behaviors/<feature>/code-agent-index.json` and are excluded from validation.
