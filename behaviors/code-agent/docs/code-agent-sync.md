...
- ✅ Merges `tasks.json` files from `behaviors/*/.vscode/tasks.json` into root `.vscode/tasks.json`
...
behaviors/<feature-name>/
  ...
  ├── <function-name>.py        → command-runners/ (root level, NOT .cursor/command-runners/)
...
- `.py` files → `command-runners/` (project root, not inside `.cursor/`)
- `.json` files → `.cursor/mcp/` (merged for MCP configs)

**Special Handling:**
- `behaviors/*/.vscode/tasks.json` → Merged into root `.vscode/tasks.json` (tasks combined, duplicate labels avoided)
