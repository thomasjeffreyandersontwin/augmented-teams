---
title: Character Prompt Template - Non-Combat Mode
description: Parameterized prompt structure for character agents in non-combat mode
feature: character
---

# Character Prompt Template - Non-Combat Mode

This template defines the structure for building prompts in non-combat mode. Parameters are filled from generated character profiles (created by `/character-generate`).
...
{mode-transition-announcement} (e.g., "Entering non-combat mode" - only included if mode changed)
...
Write as {character-name} in their {selected-identity-name} identity ({selected-identity-type}). Write naturally, as {character-name} would speak or think in this identity.
...
- Include reasonable drama and suspense. Minor speech or thought can accentuate the activity.
- If output_type is `act`, ensure actions align to `{non-combat-act-examples}` and do not resemble `{non-combat-act-bad-examples}`.
- If output_type is `speak`, ensure speech aligns to `{non-combat-speak-examples}` and does not resemble `{non-combat-speak-bad-examples}`.
- If output_type is `both`, use both speak and act examples together; ensure actions align to `{non-combat-act-examples}` and speech to `{non-combat-speak-examples}`. Action and speech should complement each other without redundancy.
...
Go through the examples in `behaviors/character/characters/{character-name}/character-profile.mdc` -> Narrative Style Examples section -> non-combat, {output_type} subsection -> Examples (What to say) subsection for reference examples of what to say:
{from character profile: non-combat-{output_type}-examples}

**Style Bad Examples (What NOT to say):**
Go through the bad examples in `behaviors/character/characters/{character-name}/character-profile.mdc` -> Narrative Style Examples section -> non-combat, {output_type} subsection -> Bad Examples (What NOT to say) subsection for reference examples of what NOT to say:
{from character profile: non-combat-{output_type}-bad-examples}
