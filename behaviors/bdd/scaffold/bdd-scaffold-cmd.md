---
execution:
  registry_key: bdd-scaffold
  python_import: behaviors.bdd.bdd_runner.BDDScaffoldCommand
  cli_runner: behaviors/bdd/bdd_runner.py
  actions:
    generate:
      cli: generate-scaffold
      method: generate
    validate:
      cli: validate-scaffold
      method: validate
    correct:
      cli: correct-scaffold
      method: correct
...
2. **AI Agent** (using `BDDScaffoldCommand.generate()`) determines the test file path (from user input or context) and uses `behaviors/code-agent/utils/command_executor    - Calls `execute_command("bdd-scaffold", "generate", **params)` where params include test_file_path
...
4. **Runner** (`BDDScaffoldCommand.generate()`) which generates the scaffold hierarchy:
...
2. **AI Agent** determines the test file to validate (from user input or context) 3. **Runner** (`BDDScaffoldCommand.validate()`) which validates if the scaffold hierarchy follows the principles:
...
2. **AI Agent** reads scaffold file and validation errors (if any), plus chat context provided by user,    - Calls `execute_command("bdd-scaffold", "correct", **params)` where params include test_file_path and chat_context
   - The executor automatically parses execution metadata, tries Python import first, falls back to CLI if needed
