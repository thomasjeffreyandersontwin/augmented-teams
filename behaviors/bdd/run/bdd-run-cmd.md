---
execution:
  registry_key: bdd-run
  python_import: behaviors.bdd.bdd_runner.RunCommand
...
2. **AI Agent** uses `behaviors/code-agent/utils/command_executor.py` to execute the command automatically:
   - Calls `execute_command("bdd-run", "generate", **params)` where params are determined from user input or context
