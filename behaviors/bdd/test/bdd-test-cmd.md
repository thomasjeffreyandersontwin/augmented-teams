---
execution:
  registry_key: bdd-test
  python_import: behaviors.bdd.bdd_runner.TestCommand
...
2. **AI Agent** uses `behaviors/code-agent/utils/command_executor.py` to execute the command automatically:
   - Calls `execute_command("bdd-test", "generate", **params)` where params are determined from user input or context
