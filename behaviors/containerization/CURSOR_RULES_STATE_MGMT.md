...
```
python behaviors/containerization/provisioner.py AZURE behaviors/vector-search --always
```
...
```
python behaviors/containerization/delivery-pipeline.py run --feature vector-search --mode AZURE
```
...
check_deployment:
  ...
  command: |
    python behaviors/containerization/delivery-pipeline.py status \
...
```bash
# 1. Check what failed
python behaviors/containerization/delivery-pipeline.py status --feature failing-feature

# 2. Look at state file
cat behaviors/failing-feature/.deployment-state.json

# 3. Fix the issue
# ... make changes ...

# 4. Resume from failure point
python behaviors/containerization/delivery-pipeline.py resume --feature failing-feature
```
...
python behaviors/containerization/delivery-pipeline.py run --feature <feature-name> --mode AZURE

@cmd check-pipeline <feature-name>  
python behaviors/containerization/delivery-pipeline.py status --feature <feature-name>

@cmd pause-pipeline <feature-name>
python behaviors/containerization/delivery-pipeline.py pause --feature <feature-name>

@cmd resume-pipeline <feature-name>
python behaviors/containerization/delivery-pipeline.py resume --feature <feature-name>

@cmd reset-pipeline <feature-name>
python behaviors/containerization/delivery-pipeline.py clear --feature <feature-name>

# Quick provision without state management
@cmd quick-provision <feature-name>
python behaviors/containerization/provisioner.py AZURE behaviors/<feature-name> --always
```
...
cp behaviors/mcp-proxy/.deployment-state.json behaviors/mcp-proxy/.deployment-state.json.backup

# 2. Deploy
python behaviors/containerization/delivery-pipeline.py run --feature mcp-proxy

# 3. If failed, restore
cp behaviors/mcp-proxy/.deployment-state.json.backup behaviors/mcp-proxy/.deployment-state.json
```

### Pattern 2: Test Before Deploy
```bash
# Test locally first
python behaviors/containerization/provisioner.py SERVICE behaviors/new-feature
python behaviors/new-feature/service-test.py SERVICE

# Then deploy to Azure
python behaviors/containerization/delivery-pipeline.py run --feature new-feature --mode AZURE
```

### Pattern 3: Dry Run
```bash
# Check what would happen (without deploying)
python behaviors/containerization/delivery-pipeline.py status --feature test-feature
...
# AI checks status
python behaviors/containerization/delivery-pipeline.py status --feature vector-search
