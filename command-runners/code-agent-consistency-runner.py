def behavior_consistency(feature=None):
    ...
    try:
        ...
        # Try to load from behaviors/.env first, then root .env
    ...
    if not api_key:
        ...
        print("   Set it in behaviors/.env or .env file, or with: export OPENAI_API_KEY='your-key'")
        return
    ...
    for behavior in behaviors:
        ...
        if not file_path.exists():
            # Try alternative path - use the path from behavior data directly
            if behavior["location"] == "features" or behavior["location"] == "behaviors":
                file_path = Path("behaviors") / behavior["feature"] / behavior["file"]
            else:
                # Try deployed location
                if behavior["location"].startswith(".cursor/command-runners/"):
                    file_path = Path(behavior["location"].replace(".cursor/", ".cursor/")) / behavior["file"]
                elif behavior["location"].startswith("command-runners/"):
...
def behavior_consistency_watch():
    ...
    Monitors behaviors/*/code-agent-behaviors/ directories for changes to:
    - .mdc files (rules)
    - .md files (commands)
    ...
    # Set up watcher - find all directories with behavior.json where deployed=true
    ...
    if not watch_dirs:
        print("⚠️  No behaviors with deployed=true found in behaviors/")
        return
    
    for watch_dir in watch_dirs:
        observer.schedule(event_handler, str(watch_dir), recursive=True)
        notify(f"👀 Watching: {watch_dir}")
